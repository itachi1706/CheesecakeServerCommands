    ...
    public void execute(@Nonnull MinecraftServer server, @Nonnull ICommandSender iCommandSender, @Nonnull String[] args) {
        ...
        if(args.length == 0)
        {
            if (!PlayerMPUtil.isPlayer(iCommandSender)) {
                ...
                if (player == null) {
                    ChatHelper.sendMessage(iCommandSender, "Cannot set " + iCommandSender.getName() + " as Survival Mode");
                    return;
                }
                player.setGameType(GameType.SURVIVAL);
                ChatHelper.sendMessage(iCommandSender, "Set own gamemode to " + TextFormatting.GOLD + "Survival Mode");
        ...
        player.setGameType(GameType.SURVIVAL);
        ChatHelper.sendMessage(iCommandSender, "Set " + player.getName() + " gamemode to " + TextFormatting.GOLD + "Survival Mode");
        ChatHelper.sendAdminMessage(iCommandSender, "Set " + player.getName() + " gamemode to Survival Mode");
    }
